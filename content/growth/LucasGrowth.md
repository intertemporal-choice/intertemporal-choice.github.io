...
<!-- Figure placeholder: LucasGrowthFig - to be generated -->
...
and it is obvious that if {math}`\CRRA>1` as we usually assume,
...
capital in this model is always {math}`A = r`, because {math}`f'(K) = \PtyLev`. Note
...
We can also use {eq}`eq:LucasAK-kdot` to obtain an expression for the

steady-state growth rate:

```{math}
:label: eq:LucasAK-ssg
