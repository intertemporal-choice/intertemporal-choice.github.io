...
 {cite:t}`solow:contribution`, and Cass {cite:t}`cass:growth`-Koopmans {cite:t}`koopmans:growth`, attributes virtually all long-run growth to technological
...
:::{margin}
*Impossible* in CRS perf comp no externalities framework to incorporate an economic theory of {math}`g`, because all income goes to {math}`K` and {math}`L`.
:::
...
paper by Paul Romer {cite:t}`romer:growth` led the way to the formulation
...
paper by Sergio Rebelo {cite:t}`rebelo:long` provided a succinct summary
...
:::{margin}
Solow: {math}`\kapShare = (1-\labShare) \rightarrow` 2nd term in {eq}`eq:AKeqn` vanishes
:::
...
:::{margin}
SS growth impossible with {math}`\labShare+\kapShare \neq 1` and {math}`\popGro \neq 0`
:::
...
:::{margin}
Return to lecture notes for discussion of 2.a.ii, increasing returns growth models.
:::
