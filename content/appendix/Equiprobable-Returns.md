...
of the levels are independent of the size of the risk (cf. [Math Facts](#fact:mathfactslist)). That is, defining {math}`\ShkMeanOne_{i} = \Ex_{t}[e^{\ShkMeanOneLog_{i,t+1}}]` for {math}`i \in \{1,2\}`:
...
```{math}
\begin{gathered}\begin{aligned}
          \risky_{1,t+1} & = \overbrace{\risky_{1}+0.5(\scale \sigAll)^{2}}^{\equiv \riskyAlt_{1}}+\ShkMeanOneLog_{1,t+1}
...
```{math}
\begin{gathered}\begin{aligned}
  \Risky_{1,t+1} & \equiv e^{\risky_{1,t+1}}
\\ \Risky_{1} \equiv\Ex_{t}[\Risky_{1,t+1}] & = e^{\risky_{1}+(\scale \sigAll)^{2}/2}
\\ \log \Ex_{t}[\Risky_{1,t+1}] & = \risky_{1}+(\scale \sigAll)^{2}/2 = \riskyAlt_{1}
\end{aligned}\end{gathered}
```

where note to avoid confusion that {math}`\riskyAlt_{1} \neq \Ex_{t}[{{\risky}}_{1,t+1}]`
...
\begin{gathered}\begin{aligned}
   \risky_{2,t+1} & \equiv \riskyAlt_{2}+\zeta + (\omega /\scale) \ShkMeanOneLog_{1,t+1} + \ShkMeanOneLog_{2,t+1}
...
\begin{gathered}\begin{aligned}
  \cov({\risky}_{1,t+1} ,{\risky}_{2,t+1}) & = \cov({\risky}_{1,t+1} , (\omega/\scale){\risky}_{1,t+1} )
...
```{math}
\begin{gathered}\begin{aligned}
  \Risky_{2} \equiv \Ex_{t}[\Risky_{2,t+1}] & = e^{\riskyAlt_{2}}
\end{aligned}\end{gathered}
```
...
```{math}
\begin{gathered}\begin{aligned}
   \risky_{2,t+1} & = \riskyAlt_{2}+0.5 (\omega \scale-\omega^{2}) \sigAll^{2} + (\omega /\scale) \ShkMeanOneLog_{1,t+1} + \ShkMeanOneLog_{2,t+1}
...
```{math}
\begin{gathered}\begin{aligned}
 \text{corr}({\risky}_{1,t+1} ,{\risky}_{2,t+1}) & \equiv \cov({\risky}_{1,t+1} ,{\risky}_{2,t+1})/\sigma_{1}\sigma_{2}
\\ & = \omega/(1+\omega^{2})^{0.5} .
\end{aligned}\end{gathered}
```
...
Standard numerical computation software can compute this double integral, but at such a slow speed as to be unusable for many purposes.

[^YO5HNPAPo4]: Under the lognormal assumption, {math}`\underline{\tilde{\Risky}}_{i}=0` and {math}`\bar{\Risky}_{i}=\infty` for both shocks {math}`i \in \{1,2\}`.
...
:::{figure} /content/figures/Equiprobable-Returns/ShareVsCovByMethod.png
