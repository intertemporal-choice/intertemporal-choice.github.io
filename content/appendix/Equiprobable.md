...
[^gOxRS4n81Q]: The assumed distribution has the property {math}`\Ex[\ShkMeanOne_{1,t+1}]=1`, cf. .
...
:::{admonition} Alternative representation (commented out)
:class: dropdown

One way to represent the joint distribution would be to stack the two
variables {math}`\ShkMeanOneLog_{1,t+1}` and {math}`\ShkMeanOneLog_{2,t+1}` into a vector {math}`\vec{\ShkMeanOneLog}` and to write
...
where {math}`\mu= \{-0.5 \sigma^{2}_{1},-0.5 \sigma^{2}_{2}\}'` and the covariance matrix
{math}`\Sigma` is diagonal with variances {math}`\sigma^{2}_{1}` and {math}`\sigma^{2}_{2}`.[^fnEquip]

[^fnEquip]: An alternative would be to work with {math}`\ShkMeanOneLog_{1,t+1}` and {math}`\risky` directly, which would require a multivariate normal with nonzero off-diagonal elements (covariances). The two approaches are mathematically indistinguishable, but the IID representation has certain conveniences for our purposes.
:::
...
:::{figure} /content/consumption/Equiprobable/LaTeX/Figures/CDFPlot.png
:name: fig:corr0p5
