...
:::{margin}
In continuous-time problems there is an implicit {math}`(t)` argument that is never written out.
:::
...
usually obtain by using the Envelope theorem in the discrete-time problem,

:::{margin}
The only real difference is {math}`x_{t}` versus {math}`\kap_{t}`; {math}`\kap_{t}` is the right measure in a continuous-time model.
:::
...
:::{margin}
Note that {math}`\dot{\lambda}/\lambda` is expressed at an annual rate.
:::
...
[^h2DGSZMi5A]: Note that this also rules out negative {math}`\kap_{t}` values that grow faster than the interest rate.
