...
class KeynesianConsumer:
    ...
    def __init__(self):
        ...
        )  # set up a consumer type and use default parameters
...
```{code-cell} ipython3
# Let's have a look at some aggregate data
...
```{code-cell} ipython3
# Let's use our second equation to try to find an estimate of a_1
...
```{code-cell} ipython3
# Let's use the data from FRED that we used before.
