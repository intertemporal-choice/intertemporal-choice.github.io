...
:::{margin}
Since the time endowment is one, resources are {math}`\Wage_{t} \cdot 1`.
:::
...
roughly equals labor income {math}`\cons = (1-\leisure)\wage` we have

```{math}
\begin{gathered}\begin{aligned}
        \left(\frac{\wage  \leisure}{\wage (1-\leisure)}\right) & =  \left(\frac{\zeta}{1-\zeta}\right)  \\
...
where {math}`\labor_{t} = 1-\leisure_{t}` is labor supply per member of the working
population and {math}`\Labor_{t} = \labor_{t}\Hours_{t}` is aggregate labor input, and where
{math}`\Hours_{t}=\sum_{i} \hours_{i}` is the aggregate amount of
...
```{math}
\begin{gathered}\begin{aligned}
        \hat{\PtyLev}_{t} & \approx  \hat{\Inc}_{t}-\labShare \hat{\Labor}_{t} - (1-\labShare)\hat{\Kap}_{t}.
\end{aligned}\end{gathered}
```
...
```{math}
\begin{gathered}\begin{aligned}
        \kap_{t+1} & =  \daleth \kap_{t}+\PtyLev_{t}\kap_{t}^{1-\labShare}\labor_{t}^{\labShare} - \cons_{t}  \\
...
:::{list-table} Standard deviations of key variables, data versus model
:header-rows: 1
:name: tab:PrescottMoments
...
\begin{gathered}\begin{aligned}
        ...
        \hat{\leisure }_{t+1} & \approx  -\hat{\wage }_{t+1}+(\rfree_{t+1}-\DiscRate)
\end{aligned}\end{gathered}
```
...
2.  Make movements in interest rates induce a strong labor
    ...
    *   Problem: If this is the mechanism, there should at the same time be a strong consumption
        ...
        there should be a one-for-one comovement of consumption with leisure; that is, recessions should be periods of high consumption and booms should be periods
        of low consumption!
...
<!-- Switches the PDF to lettered appendix chapters. It has to be emitted from the end
of the last numbered chapter, because MyST only renders a page's heading as \chapter
when that heading is the first thing in the file. Ignored outside LaTeX exports. -->
