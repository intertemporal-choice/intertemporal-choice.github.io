...
This handout derives the equity premium puzzle
(\cite{mehraPrescottPuzzle}).  Consider a representative agent
solving the joint consumption and portfolio allocation problem:
...
$\Risky_{t+1}$ denotes the return on equities (the risky asset) held 
between periods $t$ and $t+1$, $\riskyshare_{t}$ is the share of end-of-period
savings invested in the risky asset, $\Rport_{t+1}$ is the portfolio-weighted 
rate of return, and $y_{t+1}$ is noncapital income in period $t+1$.
