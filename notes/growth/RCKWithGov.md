...
\begin{gathered}\begin{aligned}
        \dot{a}_{t} & =  r_{t} a_{t} + w_{t} - c_{t} - \Tax, 
\end{aligned}\end{gathered}
```
...
The first Hamiltonian optimization condition requires {math}`\partial  \Ham_{t}/\partial c_{t} = 0`:
...
\\      & =  k_{0}+{d}_{0}+\mathbf{Y}_{0}-\mathbf{T}_{0} .
\end{aligned}\end{gathered}
```
...
where {math}`\mathbf{z}_{t}` is the per-capita size of the lump-sum rebates,
